...
当前版本。在 v0.1 基础上新增三大能力：

**流式输出** — 逐 token 实时渲染。新增 `run_turn_stream()` 方法，通过 Generator 产出结构化的 `StreamEvent` 事件（`thinking` / `content` / `done`），CLI 可按阶段分区域显示。
...
│   ├── agent.py           # 核心 Agent 循环 + StreamEvent 流式事件
│   └── cli.py             # 交互式 CLI（Rich + 流式渲染器）
├── tools/
